...
//  MAIN BUILDER
// ════════════════════════════════════════════════════════════
type TextBox = { page: number; x: number; y: number; w: number; h: number; sample: string };

function installDebugRecorder(pdf: jsPDF): TextBox[] {
  const boxes: TextBox[] = [];
  const PT_TO_MM = 0.3527777778;
  const origText = pdf.text.bind(pdf);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (pdf as any).text = function (text: string | string[], x: number, y: number, opts?: any, ...rest: unknown[]) {
    try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return (origText as any)(text, x, y, opts, ...rest);
  };
  return boxes;
}

function drawDebugOverlay(pdf: jsPDF, boxes: TextBox[]) {
  const total = pdf.getNumberOfPages();
  // Detect collisions per page
  ...
  for (let p = 1; p <= total; p++) {
    ...
    pdf.setFont("ProposalSans", "bold");
    pdf.setFontSize(7);
    pdf.text(`DEBUG p${p} · ${boxes.filter((b) => b.page === p).length} blocks · ${flagged.size} collisions`, 3, 5.6);
  }
}
