  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-5 lg:p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h3 className="text-base font-bold text-foreground">Conversion ribbon</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[repeat(4,1fr_auto)_1fr] items-center gap-3">
        {data.map((d, i) => (
          <div key={d.stage} className="contents">
            <div className="rounded-xl border border-border bg-gradient-to-br from-background/60 to-background/20 p-3">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{STAGE_LABELS[d.stage]}</p>
              <div className="flex items-baseline justify-between gap-2 mt-1">
                <p className="text-2xl font-display font-extrabold text-foreground tabular-nums">{d.reached}</p>
              ...
              <div className="h-1.5 mt-2 bg-muted rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${
                  d.stage === "won" ? "bg-success" : d.stage === "follow_up" ? "bg-warning" : "bg-primary"
