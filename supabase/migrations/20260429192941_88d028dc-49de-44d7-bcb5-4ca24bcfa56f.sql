-- Drop existing trigger(s) on deals that use handle_deal_stage_change
...
-- BEFORE trigger: only mutate NEW row timestamps
...
-- AFTER trigger: insert history row once deal row exists
...
FOR EACH ROW EXECUTE FUNCTION public.deals_write_stage_history();